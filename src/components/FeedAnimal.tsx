...
export const FeedAnimal = ({ animal }: IFeedAnimalProps) => {
  ...
  const currentAnimal = animalsFromLs.find(
    (animal) => animal.id.toString() === paramsId
  );
  ...
  useEffect(() => {
    if (currentAnimal) {
      ...
      const timeToFeed = feedingTime.plus({ minutes: 180 });
  ...
  const formatTime = DateTime.fromISO(animalToFeed.lastFed).toLocaleString(
  ...
  return (
    <>
      ...
      {!animalToFeed.isFed ? (
        <button onClick={getTime} disabled={animalToFeed.isFed}>
          Mata
        </button>
      ) : (
        <button onClick={getTime} disabled={animalToFeed.isFed}>
